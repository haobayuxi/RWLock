...
class QP {
 public:
  ...
  void bind_local_mr(MemoryAttr attr) {
    RDMA_LOG(INFO) << "qp local key " << attr.key;
    local_mr_ = attr;
  }
...
class RRCQP : public QP {
 public:
  ...
  RRCQP(RNicHandler* rnic, QPIdx idx, MemoryAttr local_mr) : RRCQP(rnic, idx) {
    bind_local_mr(local_mr);
  }

  RRCQP(RNicHandler* rnic, QPIdx idx) : QP(rnic, idx) {
  ...
  ConnStatus post_send_to_mr(MemoryAttr& local_mr, MemoryAttr& remote_mr,
    ...
    if (rc != 0) {
      RDMA_LOG(ERROR) << "ibv_post_send FAIL rc = " << rc << " "
                      << strerror(errno);
    }
  ...
  ConnStatus post_cas(char* local_buf, uint64_t off, uint64_t compare,
    ...
    return post_atomic<IBV_WR_ATOMIC_CMP_AND_SWP>(local_buf, off, compare, swap,
                                                  flags, wr_id);
  }

  // one-sided fetch and add
  ConnStatus post_faa(char* local_buf, uint64_t off, uint64_t add_value,
                      int flags, int wr_id = 0) {
    return post_atomic<IBV_WR_ATOMIC_FETCH_AND_ADD>(
        local_buf, off, add_value, 0 /* no swap value is needed*/, flags,
        wr_id);
  }

  template <ibv_wr_opcode type>
  ConnStatus post_atomic(char* local_buf, uint64_t off, uint64_t compare,
                         uint64_t swap, int flags, uint64_t wr_id = 0) {
