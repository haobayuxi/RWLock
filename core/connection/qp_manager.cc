    ...
    do {
      ...
      if (rc == SUCC) {
        data_qp->bind_remote_mr(
            meta_man->remote_hash_mrs[0]);  // Bind the hash mr as the default
                                            // remote mr for convenient
                                            // parameter passing
        ...
        //                << "key " << meta_man->remote_hash_mrs[0].key;
      }
      usleep(2000);
    } while (rc != SUCC);
    ...
    do {
      ...
      if (rc == SUCC) {
        log_qp->bind_remote_mr(
            meta_man->remote_log_mrs[0]);  // Bind the log mr as the default
        remote
